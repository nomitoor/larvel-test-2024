...
import SendMessage from "@/dashoboard/pages/Dashboard/components/SendMessage";
...
type IProp = {
    ...
    activeUserMessage: UserMessages|null
}

const ActiveChat: React.FC<IProp> = ({activeUser, activeUserMessage}) => {
        ...
        <div className="flex h-full flex-col border-l border-stroke dark:border-strokedark xl:w-3/4">
            {loading ?
                ...
                <>
                    <div className="sticky flex items-center justify-between border-b border-stroke px-6 py-4.5 dark:border-strokedark">
                        <div className="flex items-center">
                            <div className="mr-4.5 h-13 w-full max-w-13 overflow-hidden rounded-full">
                                <img
                                    src={`https://placehold.co/200X200/000000/FFF?text=${activeUser?.initial}&font=Montserrat`}
                                    alt="profile"
                            ...
                            <div>
                                ...
                                <p className="text-sm">Reply to message</p></div>
                            </div>
                        <div>
                            <div className="relative flex">
                                ...
                                <div
                                    className="absolute right-0 top-full z-40 w-40 space-y-1 rounded-sm border border-stroke bg-white p-1.5 shadow-default dark:border-strokedark dark:bg-boxdark hidden">
                                    <button
                    ...
                    <div className="no-scrollbar h-full max-h-full space-y-3.5 overflow-auto px-6 py-7.5">
                        {activeUserMessage?.map((message: UserMessages, i: React.Key | null | undefined) => (
                            message.sender_id  === user.id ? (
                                <div className="ml-auto max-w-125" key={i}>
                                    <div className="mb-2.5 rounded-2xl rounded-br-none bg-primary py-3 px-5">
                                        <p className="text-white">{message.message}</p>
                                    </div>
                                    <p className="text-right text-xs">{message.human_readable_date}</p>
                                </div>
                            ): (
                                <div className="max-w-125" key={i}>
                                    <p className="mb-2.5 text-sm font-medium">{activeUser?.username}</p>
                                    <div
                                        className="mb-2.5 rounded-2xl rounded-tl-none bg-gray py-3 px-5 dark:bg-boxdark-2">
                                        <p>{message.message}</p>
                                    </div>
                                    <p className="text-xs">{message.human_readable_date}</p>
                                </div>
                            )
                        ))}
                    </div>
                    <SendMessage activeUser={activeUser}/>
                </>
            )}
        </div>
    );
